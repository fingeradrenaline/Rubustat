...
class rubustatDaemon(Daemon):
    ...
    def cool(self):
	#Change this to false if cooling broken
	#This change to true should eliminate the need to change wiring.
	...
	roomate_2 = "gone"
		...
		if mode == "heat":
			...
			if now < monday_on:
			    scheduled = False
			elif now > monday_on and now < monday_off:
			    scheduled = True
			elif now > monday_off:
			    scheduled = False
		    elif day_of_week == 1:
			if now < tuesday_on:
			    scheduled = False
			elif now > tuesday_on and now < tuesday_off:
			    scheduled = True
			elif now > tuesday_off:
			    scheduled = False
		    elif day_of_week == 2:
			if now < wednesday_on:
			    scheduled = False
			elif now > wednesday_on and now < wednesday_off:
			    scheduled = True
			elif now > wednesday_off:
			    scheduled = False
		    elif day_of_week == 3:
			if now < thursday_on:
			    scheduled = False
			elif now > thursday_on and now < thursday_off:
			    scheduled = True
			elif now > thursday_off:
			    scheduled = False
		    elif day_of_week == 4:
			if now < friday_on:
			    scheduled = False
			elif now > friday_on and now < friday_off:
			    scheduled = True
			elif now > friday_off:
			    scheduled = False
		    elif day_of_week == 5:
			if now < saturday_on:
			    scheduled = False
			elif now > saturday_on and now < saturday_off:
			    scheduled = True
			elif now > saturday_off:
			    scheduled = False
		    elif day_of_week == 6:
			if now < sunday_on:
			    scheduled = False
			elif now > sunday_on and now < sunday_off:
			    scheduled = True
			elif now > sunday_off:
			    scheduled = False
		...
		if unread_count != 0:
			...
			if response['items'][i].title == "roomate_1 exited":
			    roomate_1 = "gone"
			elif response['items'][i].title == "roomate_1 entered":
			    roomate_1 = "here"
			elif response['items'][i].title == "roomate_2 exited":
			    roomate_2 = "gone"
			elif response['items'][i].title == "roomate_2 entered":
			    roomate_2 = "here"

			if response['items'][i].title == "roomate_2 entered" and roomate_1 == "gone":
			    ...
			    body = "The A/C will be turned up to " + str(off_temp) + " when Andrew leaves. It is currently " + str(indoorTemp) + " in the house."
			    recipient = roommate_1_mail
			    self.sendErrorMail(subject, body, recipient)
			    ...
			    recipient = roommate_1_mail
			    self.sendErrorMail(subject, body, recipient)
			elif response['items'][i].title == "roomate_1 entered" and roomate_2 == "gone":
			    subject = "Andrew Entered"
			    body = "The A/C has been turned down to " + str(on_temp) + ". It is currently " + str(indoorTemp) + " in the house."
			    recipient = roommate_1_mail
			    self.sendErrorMail(subject, body, recipient)
			elif response['items'][i].title == "roomate_1 exited" and roomate_2 == "here":
			    subject = "Andrew Exited"
			...
			if unread_count != 0:
			    ...
			    obj.store(data[i].replace(' ',','),'+FLAGS','\Seen')

		    if roomate_1 == "gone" and roomate_2 == "gone":
			    scheduled = False
		    elif roomate_1 == "here" and roomate_2 == "gone":
			    scheduled = True
		    elif roomate_1 == "gone" and roomate_2 == "here":
			    scheduled = True
		    elif roomate_1 == "here" and roomate_2 == "here":
			    scheduled = True
		...
		if mailEnabled == True and (float(targetTemp) - indoorTemp ) < errorThreshold:
            ...
            if logElapsed > datetime.timedelta(minutes=2) and sqliteEnabled:
		try:
		    import re
		    import urllib2
		    response = urllib2.urlopen("http://www.wunderground.com/weather-forecast/YOUR_URL.html")
		    page_source = response.read()
		    result = re.search('Station, TX\ \| (.*)&deg', page_source)
		    outsidetemp = float(result.group(1))
			...
			total += temp_f_i
			if i == 0:
			    roomate_1room = temp_f_i
			elif i == 1:
			    roomate_2room = temp_f_i
			elif i == 2:
			    kitchen = temp_f_i


		    c.execute("INSERT INTO logging VALUES (?,?,?,?,?,?,?,?,?);", (now,indoorTemp,targetTemp,roomate_2,roomate_1,roomate_1room,roomate_2room,kitchen,outsidetemp))
		    conn.commit()
		    lastLog = datetime.datetime.now()
		...
		#Who is home?
		log.write("roommateStatus = roomate_2: " + str(roomate_2) + " roomate_1: " + str(roomate_1) +"\n")
...
if __name__ == "__main__":
        ...
        if sqliteEnabled == True:
            ...
            c = conn.cursor()
            #c.execute('''CREATE TABLE IF NOT EXISTS logging (datetime TIMESTAMP, actualTemp FLOAT, targetTemp INT, roomate_2 TEXT, roomate_1 TEXT)''')
